  ...
  for (int i = 0; i < m_nUser; i++)
    {
      ...
      double expectedMinDlTxPackets = statsDuration.GetMilliSeconds () - 50;
      NS_TEST_ASSERT_MSG_GT (dlTxPackets, expectedMinDlTxPackets,
                             " too few DL TX packets reported");

      // the quantiles are evaluated offline according to a Bernoulli 
      // distribution with n equal to the number of packet sent and p equal 
  ...
  for (int i = 0; i < nUser; i++)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ_TOL (dlBler, m_blerRef, 0.1, " Unexpected DL BLER distribution!");
    }
