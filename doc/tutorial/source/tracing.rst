...
container to get a Ptr<Node> which we used to call GetId() on.  We
could have used this Ptr<Node> directly to call a connect method
directly::
...
find a Ptr<Object> and call the appropriate TraceConnect method at the
lowest level.
...
This reference is actually a ``Ptr<Node>`` and so is a subclass of an
``ns3::Object``.
...
from the original Ptr<Node> as specified by "/NodeList/7" to its
...
to the project web site, `ns-3 project <http://www.nsnam.org>`_, you
...
that returns void and takes a a ``Ptr<const MobilityModel>``.  For
example,
...
of the Callback.  The second (non-optional) parameter, ``Ptr<const
MobilityModel>`` represents the first argument to the callback.
...
will correspond to a ``TracedCallback<int32_t, int32_t>`` when the
...
single parameter which is a ``Ptr<const Packet>`` -- just what we have
above.
...
integrated into the config system.  The Ptr<OutputStreamWrapper>, on
...
<prefix>-<node>-<device> file name generation mechanism is replaced by
...
node/net-device pair by providing a ``Ptr<NetDevice>`` to an
``EnableAscii`` method.  The ``Ptr<Node>`` is implicit since the net
...
of which follows the <prefix>-<node id>-<device id>.tr convention.
...
of these files will follow the <prefix>-<node id>-<device id>.tr
...
follows the <prefix>-<node id>-<device id>.tr convention.  Combining
